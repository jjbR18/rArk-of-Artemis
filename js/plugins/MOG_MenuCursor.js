...
Scene_Base.prototype.create_menucursor = function() {
    ...
    this._sprite_mcursor.visible = false;
    $gameTemp._preparedCursor = false;
};
...
Scene_Base.prototype.update_mcursor = function() {
	...
	if ($gameTemp._mcursorData[1] > 0) {$gameTemp._mcursorData[1] -= 1,this._sprite_mcursor.opacity +=45;}
	...
	var ny = $gameTemp._mcursorData[3] + this._mcursor_data[1];
    this._sprite_mcursor.x = this.cursorMoveToSVD(this._sprite_mcursor.x,nx,5);
	this._sprite_mcursor.y = this.cursorMoveToSVD(this._sprite_mcursor.y,ny,5);
	if ($gameTemp._preparedCursor && !this._sprite_mcursor.visible) {
        this._sprite_mcursor.x = nx;
        this._sprite_mcursor.y = ny;
        this._sprite_mcursor.visible = true;
	}
};
