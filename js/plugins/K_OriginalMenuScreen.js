...
(function() {
	...
	var Information_Msg = {
		...
		'eroStatus': String(parameters['Information_tips'] || '确认迄今为止的性经验。'),
		'glossary1': String(parameters['Information_glossary1'] || '阅览有助于冒险的信息。'),
		'options': String(parameters['Information_options'] || 'オプション画面を開きます。'),
		'save': String(parameters['Information_save'] || 'これまでのデータをセーブします。'),
		'achievement': String(parameters['Information_Achievement'] || '确认已获得的成就。')	}
	...
	Window_Information.prototype.refresh = function() {
	    ...
	    if (Add_LocationInfo == 1) {
		    this.drawTextEx(this._text, this.textPadding(), this.fittingHeight(0));
		   	...
		   	this.drawText('场所', this.textPadding(), 0, 56, 'left');
		   	this.resetTextColor();
		    this.drawText($gameMap.displayName(), 62 + this.standardPadding(), 0, 280, 'left');
		    // プレイ時間
		    this.changeTextColor(this.systemColor());
		   	this.drawText('游戏时长', 360 + this.standardPadding(), 0, 140, 'left');
		   	this.resetTextColor();
